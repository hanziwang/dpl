			...
			<form class="form">
				...
				<div class="field clearfix">
					...
					<div class="clearfix types">
						<input type="hidden" name="category" required="required">
						<span class="cat-selected"></span>
						<a href="javascript:;" class="cat-select">点此选择分类 »</a>
						<div class="cat-unselected" style="display:none;">
...
<script>
	...
	$('.cat-select').on('click', function () {
		$('.cat-unselected').fadeToggle(0.25);
	});

	// 删除已选分类
	$('.cat-selected').on('click', 'a', function () {
		$(this).parent().remove();
	});

	// 添加可选分类
	$('.cat-unselected').on('click', 'a', function () {

		var id = $(this).attr('data-id'),
			text = $(this).text();

		if ($('#cat-selected-' + id).length === 0) {
			$('.cat-selected').append('<span id="cat-selected-' + id + '"><a data-id="' + id + '" href="javascript:;"></a>' + text + '</span>');
		}
